$(document).on 'turbolinks:load', ->
  #event suring keypress, enter and validation email and name
...
#event focusout
  ...
  #when link delete clicked, call member delete
  ...
  $('.new_member').on 'submit', (e) ->
    $.ajax e.target.action,
        ...
        success: (data, text, jqXHR) ->
          ...
          $('#member_name, #member_email').val("") #clean fields
          $('#member_name').focus()
...
#insert member, inject html
insert_member = (id, name, email) ->
  $('.member_list').append( #add in final
